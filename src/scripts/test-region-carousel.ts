/**
 * 인천 지역 매출 조회 및 캐러셀 메시지 전송 테스트 (v6)
 *
 * 개선사항:
 * 1. 텍스트 박스에 둥근 모서리 + 흰색 배경
 * 2. 회색 텍스트 색상 진하게 (#666666)
 * 3. 마지막 버블 소수점 제거 + 백만단위 유지
 * 4. 품목별 매출액 진하게 + 더보기 버튼 추가
 */
...
const TARGET_USER_ID = '73524122-e756-4c53-179e-0378b4ad90b5';
const SEARCH_KEYWORD = '인천';
...
async function getRegionSales(keyword: string): Promise<{
  ...
  console.log(`데이터 범위: ${dataRange.min_index} ~ ${dataRange.max_index}`);

  // 데이터가 있는 가장 최근 3개월 사용
  ...
  // 조회 기간 텍스트 생성 (예: "2025.10 ~ 2025.12")
  ...
  console.log(`조회 기간: ${startIndex} ~ ${endIndex} (${periodMonths}개월) - ${periodText}`);
...
async function main() {
  console.log('=== 인천 지역 매출 조회 테스트 (v6) ===\n');
  console.log(`입력: "${SEARCH_KEYWORD}"`);
  console.log(`대상 유저: ${TARGET_USER_ID}\n`);

  if (SEARCH_KEYWORD.length < 2) {
    console.log('2글자 이상 입력해주세요.');
    return;
  }
  console.log('입력 검증 통과 (2글자 이상)\n');

  try {
    await getConnection();

    console.log('DB 조회 중...');
    const { summary, topHospitals, topDrugs, periodMonths, periodText } = await getRegionSales(SEARCH_KEYWORD);
    ...
    console.log(`평균: ${formatMoney(monthlyAvg)}`);
    console.log(`추이: (${trendText})`);
    console.log(`병원: ${summary.hospital_count}개, 품목: ${summary.drug_count}개`);

    console.log('\nTOP 5 병원:');
    topHospitals.forEach((h, i) => {
      const avg = h.total_sales / periodMonths;
      const trend = formatMonthlyTrend(h.monthlySales);
      console.log(`\n  ${i + 1}. ${h.hos_abbr || h.hos_name}`);
      console.log(`     평균: ${formatMoney(avg)} (${trend})`);
      h.drugSales.slice(0, 3).forEach(d => {
        console.log(`     - ${d.drug_name}: ${formatMoney(d.total_sales)}`);
      });
      if (h.drugSales.length > 3) {
        const otherTotal = h.drugSales.slice(3).reduce((s, d) => s + d.total_sales, 0);
        console.log(`     - 기타 ${h.drugSales.length - 3}품목: ${formatMoney(otherTotal)}`);
      }
    });

    console.log('\nTOP 5 품목 (지역):');
    topDrugs.forEach((d, i) => {
      console.log(`  ${i + 1}. ${d.drug_name}: ${formatMoneyInt(d.total_sales)}`);
    });

    const carousel = createRegionCarousel(SEARCH_KEYWORD, summary, topHospitals, topDrugs, periodMonths, periodText);
